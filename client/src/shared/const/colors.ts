export const base = "#764cff";
export const base2 = "#0e0430";
export const base3 = "#283b8b";

export const white = "#f1f1f4";
export const textImportant = "#ff6347";
export const textDark = "#333";
export const textSecondary = "#d2d2d2";

export const grey = "#ddd";
export const black = "#222";

export const primary = "#ffef44";
export const secondary = "#8a2be2";
export const danger = "#f0301b";

export const modalBg2 = "#e25f5f";

export const notificationBg = "#604e97";
export const notificationWarning = textImportant;
export const notificationSuccess = "greenyellow";

export const spinnerContrasting = textImportant;

export const opaqueBlack = "rgba(0, 0, 0, 0.2)";
export const disabled = "#858585";

export const dashboardPageBg = "rgb(106 72 220)";
export const clientsPageBg = "#116D6E";

export const dashboardModalBg = "rgb(120 92 211)";

export const greenLight1 = "rgb(60, 139, 90)";
export const greenLight2 = "#4db575";
export const greenDark1 = "#3d6a4e";
export const greenDark2 = "#49815e";
