...
const initialState = projectsAdapter.getInitialState<IProjectState>({
  status: projectLoadingStatus.loading,
});
...
export const projectsSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(fetchProjects.pending, (state, _) => {
      state.status = projectLoadingStatus.loading;
    });
    builder.addCase(fetchProjects.fulfilled, (state, action) => {
      state.status = projectLoadingStatus.fulfilled;
      projectsAdapter.upsertMany(state, action.payload);
    });
    builder.addCase(fetchProjects.rejected, (state) => {
      state.status = projectLoadingStatus.failed;
    })
